...
export default function App() {
  ...
  const hasNextTrack = usePlayerStore((state) => Boolean(getNextQueueState(state)));
  ...
  const displayedTracks = activeView === "favorites" ? favoriteTracks : tracks;
  ...
  return (
    <main className="app-shell">
      <LibrarySidebar
        activeFolderId={activeView === "folder" ? selectedFolderId : null}
        ...
        <TrackTable
          ...
          onSelectTrack={(track) => {
            const sourceFolderId =
              activeView === "favorites"
                ? favoriteTracks.find((favorite) => favorite.audioPath === track.audioPath)
                    ?.folderId
                : selectedFolder?.id;
            if (sourceFolderId) {
