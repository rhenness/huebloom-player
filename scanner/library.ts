...
export interface Folder {
    /** POSIX directory path relative to `music/`. */
    id: string;
    name: string;
    tracks: Track[];
}
...
function validateFolder(value: unknown, location: string): Folder {
    ...
    if (!Array.isArray(value.tracks)) {
        throw new LibraryValidationError(
            `${location}.tracks must be an array.`,
        );
    }
...
export function validateLibrary(value: unknown): Library {
    if (!isRecord(value)) {
        throw new LibraryValidationError(
            'library.json must contain an object.',
        );
    }
    ...
    if (!Array.isArray(value.folders)) {
        throw new LibraryValidationError(
            'library.json.folders must be an array.',
        );
    }
    ...
    assertUnique(
        folders.flatMap((folder) =>
            folder.tracks.map((track) => track.shareId),
        ),
...
function compareFolderIds(left: string, right: string): number {
...
export function reconcileLibrary(
    ...
    const folderIds = new Set(discoveredFolders.map((folder) => folder.id));
    ...
    const folders = [...discoveredFolders]
        .sort((left, right) => compareFolderIds(left.id, right.id))
        .map((folder) => ({
            id: folder.id,
            name: folder.name,
            tracks: [...folder.tracks]
                ...
                .map((track) => {
                    const existingTrack = existingTracksByAudioPath.get(
                        track.audioPath,
                    );
        ...
        .filter(
            (folder) =>
                ...
                [...folderIds].some((id) => id.startsWith(`${folder.id}/`)),
        );
