...
describe("scanLibrary", () => {
  ...
  it("scans direct MP3 files, ignores unrelated content, and writes a sorted manifest", async () => {
    ...
    await writeFixtureFile(repositoryRoot, "music/2025/nested/Project_3.mp3");
  ...
  it("rejects unpulled Git LFS pointer files without writing a manifest", async () => {
    ...
    await expect(scanLibrary({ repositoryRoot })).rejects.toThrow(ScanError);
    await expect(
